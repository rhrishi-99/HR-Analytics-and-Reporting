...
| Raihan Naeem | PES1UG23AM227 | Metrics engine, all MetricStrategy implementations, AnalyticsEngine | Strategy (Behavioural) |
| R G Rhrishi | PES1UG23AM222 | Charts, dashboards, reports, exports, filters | Abstract Factory (Creational) |
...
HRMS Core Modules (Employee / Payroll / Attendance / Performance)
    ...
    → MetricsCalculationEngine   (Strategy pattern lives here)
    → AnalyticsEngine
...
All three categories are required by the course; each is owned by one team member. Do not replace these patterns without updating this file and the design doc.
...
- Three public methods: `loadDashboard(userId, filters)`, `generateReport(reportType, filters)`, `exportReport(reportId, format)`.
...
### Behavioural — Strategy (MetricsCalculationEngine)
- Interface `MetricStrategy` with a single method `calculate(data: ProcessedData) → MetricResult`.
- One concrete class per metric: `AttritionRateStrategy`, `EmployeeGrowthStrategy`, `AveragePerformanceStrategy`, `DepartmentMetricsStrategy`, `CompensationAnalyticsStrategy`.
- `MetricsCalculationEngine` holds `Map<MetricType, MetricStrategy>` and delegates. No switch statements, no if-else chains on metric type.
- To add a metric: new strategy class only. Do not modify the engine.
...
**GRASP:** Information Expert (domain classes own their getters), Creator (factories/generators own construction), Controller (Facade is the system controller), Low Coupling (external access via Facade only), High Cohesion (one job per engine), Polymorphism (Chart, ExportFormat, MetricStrategy), Pure Fabrication (engines aren't domain concepts), Indirection (Facade, DataIntegrationLayer), Protected Variations (Strategy shields from metric change, Factory from chart-family change).

**SOLID:** SRP (one class, one reason to change), OCP (new metric/chart/export = new class, no edits), LSP (all strategy/chart/export subtypes are substitutable), ISP (narrow interfaces — ChartFactory, MetricStrategy, ExportFormat), DIP (engines depend on abstractions, never concrete classes).
...
The sub-system must demonstrate integration with at least two other sub-systems (DB and exception handling don't count).

### Integration 1 — Performance Management Sub-System (we consume)
- **Direction:** inbound data pull.
- **Consumer:** `DataCollectionModule` via `HRAnalyticsFacade`-internal scheduler.
...
Both partners touch only the Facade. Do not let integration code reach into pipeline internals.
...
- **Naming:** classes match the class diagram exactly (`MetricsCalculationEngine`, not `MetricsEngine` or `MetricCalculator`). Strategy classes end in `Strategy`. Factories end in `Factory`. Exception classes match the canonical names in SCREAMING_SNAKE_CASE constants or equivalent.
- **No duplicate code across owners.** If Raihan needs a utility Prem already wrote, import it — do not re-implement.
- **No switch/if-else on type.** Type-based branching is a signal that Strategy or Factory should be used instead.
- **No direct access to internal modules from outside the package.** Everything external goes through `HRAnalyticsFacade`.
- **Dependency direction:** always toward abstractions. Engines hold interface references, never concrete class references.

---

## What Claude Should Do

- When adding a new metric: create a new `MetricStrategy` subclass, register it in the engine's map, done. Never edit `MetricsCalculationEngine` logic.
- When adding a new chart type: extend `Chart`, add creation method to `ChartFactory`, implement in each concrete factory. Never edit `DashboardManager`.
...
- Do not introduce new design patterns in place of the three mandated ones without explicit team approval.
...
- `HR_Analytics_Project_KnowledgeExport.md` — full context snapshot; paste into a new chat to restore session state
...
Implementation language: **Java** (OOAD course standard). Build tooling and test commands are TBD — update this section when the project skeleton is created.
