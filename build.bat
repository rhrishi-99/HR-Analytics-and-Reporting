...
set LIB_JAR=hrms-database-1.0-SNAPSHOT-all.jar
...
%JAVAC% --release 17 -cp "%LIB_JAR%" -d %OUT% @.sources.txt
if errorlevel 1 (
...
if "%1"=="web" (
    ...
    echo.
    %JAVA% -cp "%OUT%;%LIB_JAR%" com.hranalytics.web.WebMain
) else (
    echo === HR Analytics: Running Console Demo ===
    %JAVA% -cp "%OUT%;%LIB_JAR%" com.hranalytics.Main
)
